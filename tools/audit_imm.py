...
def ev(e, depth=0):
    e = re.sub(r'GV\(([^,()]+(?:\([^()]*\))?),([^()]+)\)', r'\2' if SEASONS else r'\1', e)
    for _ in range(6):
...
for p in sorted(glob.glob('src/game/**/*.c', recursive=True)):
    ...
    for n, line in enumerate(open(p, errors='replace'), 1):
        ...
        for m in re.finditer(r'\bCYCT?\(b_\+(\d+), b_\+(\d+)\);\s*mem_wr\(gb, HL, (0x[0-9a-fA-F]+)\);', code):
        ...
        for m in re.finditer(r'\bCYCT?\(b_\+(\d+), b_\+(\d+)\);\s*SET_(BC|DE|HL)\((0x[0-9a-fA-F]+)\);', code):
