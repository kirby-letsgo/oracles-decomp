...
void parseGivenObjectData_b12_hook(GB *gb) {
  ...
  uint16_t sp0_ = gb->sp;
  CYC(b_+O(0), b_+OE(1)); A = mem_rd(gb, DE);
  CYC(b_+O(1), b_+OE(3)); alu_cp(gb, 0xfe);
  if (!(F & FZ)) CYCT(b_+O(3), b_+OE(5));
  else {
    CYC(b_+O(3), b_+OE(5));
    CYC(b_+O(5), b_+OE(6)); SET_DE(pop_effect(gb));
  }
  CYC(b_+O(6), b_+OE(7)); A = mem_rd(gb, DE);
  CYC(b_+O(7), b_+OE(9)); alu_cp(gb, 0xff);
  if (F & FZ) { CYCT(b_+O(9), b_+OE(10)); ret_effect(gb); return; }
  CYC(b_+O(9), b_+OE(10));
  CYC(b_+O(10), b_+OE(11)); SET_DE(DE + 1);
  CYC(b_+O(11), b_+OE(13)); alu_and(gb, 0x0f);
