...
// ref/oracles-disasm/object_code/seasons/interactions/bb.s.
// INTERAC_bb

static uint16_t bb_jump_table(GB *gb) {
...
static void bb_add_a_to_hl(GB *gb, uint16_t return_address) {
...
static void bb_add_double_index(GB *gb, uint16_t return_address) {
  push_effect(gb, return_address);
...
static void bb_subid2_func_772e(GB *gb) {
...
subid2_func_772e:
  ...
  CYC(b_+386, b_+387); bb_add_a_to_hl(gb, b_+387);
  CYC(b_+387, b_+389); E = INTERACTION_BASE + OBJ_COUNTER2;
  CYC(b_+389, b_+390); A = mem_rd(gb, DE);
  CYC(b_+390, b_+391); bb_add_double_index(gb, b_+391);
...
static void bb_subid2_animateTwiceAndApplySpeed(GB *gb) {
  BASE(interactionCodebb);
  uint16_t sp0_ = cpu_sp(gb); (void)sp0_;
...
static void bb_subid2_setAnimationBasedOnAngle(GB *gb) {
  BASE(interactionCodebb);
  uint16_t sp0_ = cpu_sp(gb); (void)sp0_;
  CYC(b_+196, b_+198); E = INTERACTION_BASE + OBJ_ANGLE;
  CYC(b_+198, b_+199); A = mem_rd(gb, DE);
...
void s_interactionCodebb_hook(GB *gb) {
  ...
  do { uint16_t jt_ = (bb_jump_table(gb));
    if (jt_ == b_+10) goto subid0;
    if (jt_ == b_+115) goto subid1;
    HANDOFF(HL);
  } while (0);
subid0:
...
subid2_setAnimationBasedOnAngle:
  ...
  TAIL(interactionSetAnimation);
subid2_substate1:
  CALL_L(b_+205, bb_subid2_animateTwiceAndApplySpeed, b_+208);
  ...
  CALL_L(b_+224, bb_subid2_setAnimationBasedOnAngle, b_+227);
  CYC(b_+227, b_+230);
...
subid2_substate4:
  ...
  CALL_L(b_+279, bb_subid2_func_772e, b_+282);
  CYC(b_+282, b_+285);
  TAIL(interactionIncSubstate);
subid2_substate5:
...
subid2_var77_00:
  CALL_L(b_+322, bb_subid2_animateTwiceAndApplySpeed, b_+325);
...
L_7784:
  ...
  TAIL(interactionIncSubstate);
subid2_substate7:
  CALL_L(b_+467, bb_subid2_animateTwiceAndApplySpeed, b_+470);
...
subid2_substateA:
  ...
  TAIL(interactionSetAnimation);
subid2_substateB:
  CALL_L(b_+546, bb_subid2_animateTwiceAndApplySpeed, b_+549);
...
void s_func_77e5_hook(GB *gb) {
  ...
  CYC(b_+5, b_+6);
  s_func_77eb_hook(gb); return; // falls through
}

void s_func_77eb_hook(GB *gb) {
  BASE(func_77eb);
  uint16_t sp0_ = gb->sp; (void)sp0_;
  CYC(b_+0, b_+3); SET_BC(0xff20);
  CYC(b_+3, b_+6);
