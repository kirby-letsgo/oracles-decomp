...
state8:
  ...
  if (F & FZ) { CALL_L(b_+66, enemyCode15_chooseNewDirection_hook, b_+69); } else { CYC(b_+66, b_+69); } // call z
  CALL_C(b_+69, ecom_applyVelocityForSideviewEnemyNoHoles_b0d_hook, SYM(ecom_applyVelocityForSideviewEnemyNoHoles_b0d), b_+72);
  if (F & FZ) { CALL_L(b_+72, enemyCode15_chooseNewDirection_hook, b_+75); } else { CYC(b_+72, b_+75); } // call z
