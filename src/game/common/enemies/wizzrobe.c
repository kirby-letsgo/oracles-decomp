...
#undef CYC
#undef CYCT
#define CYC(from, to) burn_rom(gb, SYMBANK(enemyCode40), (from), (to), false)
#define CYCT(from, to) burn_rom(gb, SYMBANK(enemyCode40), (from), (to), true)
...
subid2:
  ...
  CYC(b_+47, SYM(wizzrobe_state_switchHook)); wizzrobe_setAnimationFromAngle_hook(gb); return; // jp
}
...
void wizzrobe_subid0_state8_hook(GB *gb) {
  ...
  CYC(b_+8, SYM(wizzrobe_subid0_state9)); objectSetVisiblec2_hook(gb); return; // jp
}
...
void wizzrobe_subid0_state9_hook(GB *gb) {
  ...
  CYC(b_+17, SYM(wizzrobe_subid0_stateA)); wizzrobe_setAnimationFromAngle_hook(gb); return; // jp
}
...
phaseOut:
  ...
  CYC(b_+21, SYM(wizzrobe_subid0_stateB)); enemySetAnimation_hook(gb); return; // jp
}
...
void wizzrobe_subid0_stateB_hook(GB *gb) {
  ...
  CYC(b_+15, SYM(wizzrobe_subid1)); objectSetInvisible_hook(gb); return; // jp
}
...
void wizzrobe_subid1_state8_hook(GB *gb) {
  ...
  CYC(b_+25, SYM(wizzrobe_subid1_state9)); wizzrobe_setAnimationFromAngle_hook(gb); return; // jp
}
...
void wizzrobe_subid1_state9_hook(GB *gb) {
  ...
  CYC(b_+10, b_+12); L = ENEMY_BASE + OBJ_COLLISION_TYPE;
  CYC(b_+12, b_+14); mem_wr(gb, HL, (uint8_t)(mem_rd(gb, HL) | (1 << 7))); // set 7,(hl)
  CYC(b_+14, SYM(wizzrobe_subid1_stateA)); objectSetVisiblec2_hook(gb); return; // jp
}
...
gotoState8:
  CYC(b_+15, b_+16); L = E;
  CYC(b_+16, SYM(wizzrobe_removePositionReservation)); mem_wr(gb, HL, 0x08); // [state]
...
void wizzrobe_subid2_state9_hook(GB *gb) {
  ...
  CYC(b_+21, SYM(wizzrobe_subid2_stateA)); wizzrobe_setAnimationFromAngle_hook(gb); return; // jr
}
...
void wizzrobe_subid2_stateA_hook(GB *gb) {
  ...
  CYC(b_+29, SYM(wizzrobe_subid2_stateB)); objectSetVisiblec2_hook(gb); return; // jp
}
...
void wizzrobe_subid2_stateB_hook(GB *gb) {
  ...
  CYC(b_+37, SYM(wizzrobe_setAnimationFromAngle)); objectSetVisiblec2_hook(gb); return; // jp
}
...
void wizzrobe_setAnimationFromAngle_hook(GB *gb) {
  ...
  CYC(b_+11, SYM(wizzrobe_checkFlickerVisibility)); enemySetAnimation_hook(gb); return; // jp
}
...
void wizzrobe_checkFlickerVisibility_hook(GB *gb) {
  ...
  CYC(b_+5, b_+6);
  CYC(b_+6, SYM(wizzrobe_checkReachedTargetPosition)); ecom_flickerVisibility_b0e_hook(gb); return; // jp
}
...
void wizzrobe_chooseSpawnPosition_hook(GB *gb) {
  ...
  CYC(b_+5, b_+6); B = A;
  CYC(b_+6, b_+8); A = hram_rd(gb, 0xaa); // hCameraY
...
pickX:
  ...
  CYC(b_+21, b_+23);
  CYC(b_+23, b_+24); C = A;
  CYC(b_+24, b_+26); A = hram_rd(gb, 0xac); // hCameraX
  CYC(b_+26, b_+27); alu_add(gb, C);
  CYC(b_+27, b_+29); alu_and(gb, 0xf0);
  CYC(b_+29, b_+31); alu_add(gb, 0x08);
  CYC(b_+31, b_+32); C = A;
  CYC(b_+32, SYM(wizzrobe_fireEvery32Frames)); getTileCollisionsAtPosition_hook(gb); return; // jp
}
...
void wizzrobe_fireEvery32Frames_hook(GB *gb) {
  ...
  CYC(b_+8, SYM(wizzrobe_markSpotAsTaken)); ecom_spawnProjectile_b0e_hook(gb); return; // jp
}
