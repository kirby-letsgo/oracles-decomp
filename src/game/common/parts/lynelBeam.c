...
#undef CYC
#undef CYCT
#define CYC(from, to) burn_rom(gb, SYMBANK(partCode1b), (from), (to), false)
#define CYCT(from, to) burn_rom(gb, SYMBANK(partCode1b), (from), (to), true)
...
L_5174:
  ...
  CYC(b_+65, SYM(partCode1c)); objectSetVisible81_hook(gb); return; // jp
}
