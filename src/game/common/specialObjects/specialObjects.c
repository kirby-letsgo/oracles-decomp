...
static void update_special_object(GB *gb) {
  ...
  do { uint16_t jt_ = (func_410d_jump_table(gb));
    if (jt_ == SYM(specialObjectCode_link) && hook_is(gb, SYM(specialObjectCode_link), specialObjectCode_link_hook)) { specialObjectCode_link_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_transformedLink) && hook_is(gb, SYM(specialObjectCode_transformedLink), specialObjectCode_transformedLink_hook)) { specialObjectCode_transformedLink_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_linkInCutscene_b00) && hook_is(gb, SYM(specialObjectCode_linkInCutscene_b00), specialObjectCode_linkInCutscene_b00_hook)) { specialObjectCode_linkInCutscene_b00_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_linkRidingAnimal) && hook_is(gb, SYM(specialObjectCode_linkRidingAnimal), specialObjectCode_linkRidingAnimal_hook)) { specialObjectCode_linkRidingAnimal_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_minecart_b05) && hook_is(gb, SYM(specialObjectCode_minecart_b05), specialObjectCode_minecart_b05_hook)) { specialObjectCode_minecart_b05_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_ricky) && hook_is(gb, SYM(specialObjectCode_ricky), specialObjectCode_ricky_hook)) { specialObjectCode_ricky_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_dimitri) && hook_is(gb, SYM(specialObjectCode_dimitri), specialObjectCode_dimitri_hook)) { specialObjectCode_dimitri_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_moosh) && hook_is(gb, SYM(specialObjectCode_moosh), specialObjectCode_moosh_hook)) { specialObjectCode_moosh_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_maple) && hook_is(gb, SYM(specialObjectCode_maple), specialObjectCode_maple_hook)) { specialObjectCode_maple_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_companionCutscene_b00) && hook_is(gb, SYM(specialObjectCode_companionCutscene_b00), specialObjectCode_companionCutscene_b00_hook)) { specialObjectCode_companionCutscene_b00_hook(gb); return; }
    else if (jt_ == SYM(specialObjectCode_raft_b05) && hook_is(gb, SYM(specialObjectCode_raft_b05), specialObjectCode_raft_b05_hook)) { specialObjectCode_raft_b05_hook(gb); return; }
    else { HANDOFF(jt_); }
  } while (0);
}
...
void updateSpecialObjects_hook(GB *gb) {
  ...
  uint16_t sp0_ = gb->sp; (void)sp0_;
  CYC(b_+O(0), b_+OE(3)); SET_HL(wLinkIDOverride);
  CYC(b_+O(3), b_+OE(4)); A = mem_rd(gb, HL);
  CYC(b_+O(4), b_+OE(6)); mem_wr(gb, HL, 0);
  CYC(b_+O(6), b_+OE(7)); alu_or(gb, A);
  if (F & FZ) { CYCT(b_+O(7), b_+OE(9)); goto no_transformation; }
  CYC(b_+O(7), b_+OE(9));
  CYC(b_+O(9), b_+OE(11)); alu_and(gb, 0x7f);
  CYC(b_+O(11), b_+OE(14)); mem_wr(gb, w1Link_id, A);
no_transformation:
...
no_input_block:;
  ...
  CYC(b_+O(39), b_+OE(40)); alu_xor(gb, A);
  CYC(b_+O(40), b_+OE(43)); mem_wr(gb, wBraceletGrabbingNothing, A);
  CYC(b_+O(43), b_+OE(46)); mem_wr(gb, wcc92, A);
  CYC(b_+O(46), b_+OE(49)); mem_wr(gb, wForceLinkPushAnimation, A);
  CYC(b_+O(49), b_+OE(52)); SET_HL(wcc95);
  CYC(b_+O(52), b_+OE(53)); A = mem_rd(gb, HL);
  CYC(b_+O(53), b_+OE(55)); alu_or(gb, 0x7f);
  CYC(b_+O(55), b_+OE(56)); mem_wr(gb, HL, A);
  CYC(b_+O(56), b_+OE(59)); SET_HL(wLinkTurningDisabled);
  ...
  CYC(b_+O(92), b_+OE(95)); SET_HL(wLinkImmobilized);
  CYC(b_+O(95), b_+OE(96)); A = mem_rd(gb, HL);
  CYC(b_+O(96), b_+OE(98)); alu_and(gb, 0x0f);
  CYC(b_+O(98), b_+OE(99)); mem_wr(gb, HL, A);
  CYC(b_+O(99), b_+OE(100)); alu_xor(gb, A);
  CYC(b_+O(100), b_+OE(103)); mem_wr(gb, wcc67, A);
  CYC(b_+O(103), b_+OE(106)); mem_wr(gb, w1Link_var2a, A);
  CYC(b_+O(106), b_+OE(109)); mem_wr(gb, wccd8, A);
  CYC(b_+O(109), b_+OE(112)); SET_HL(wInstrumentsDisabledCounter);
  CYC(b_+O(112), b_+OE(113)); A = mem_rd(gb, HL);
  CYC(b_+O(113), b_+OE(114)); alu_or(gb, A);
  if (F & FZ) { CYCT(b_+O(114), b_+OE(116)); goto clear; }
  CYC(b_+O(114), b_+OE(116));
  CYC(b_+O(116), b_+OE(117)); mem_wr(gb, HL, alu_dec8(gb, mem_rd(gb, HL)));
clear:
  CYC(b_+O(117), b_+OE(120)); SET_HL(wGrabbableObjectBuffer);
  CYC(b_+O(120), b_+OE(122)); B = 0x10;
  CYC(b_+O(122), b_+OE(125));
  TAIL(clearMemory);
}
