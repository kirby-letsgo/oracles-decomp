...
void interactionCode4a_hook(GB *gb) {
  ...
  do { uint16_t jt_ = (introSprites1_jumpTable(gb));
    ...
    else if (jt_ == SYM(introSpritesState1) && hook_is(gb, SYM(introSpritesState1), introSpritesState1_hook)) { introSpritesState1_hook(gb); return; }
    else { HANDOFF(HL); }
  } while (0);
...
state0:
  ...
  do { uint16_t jt_ = (introSprites1_jumpTable(gb));
    ...
    else if (jt_ == SYM(introSpriteIncStateAndLoadGraphics) && hook_is(gb, SYM(introSpriteIncStateAndLoadGraphics), introSpriteIncStateAndLoadGraphics_hook)) { introSpriteIncStateAndLoadGraphics_hook(gb); return; }
...
void introSpritesState1_hook(GB *gb) {
  ...
  do { uint16_t jt_ = (introSprites1_jumpTable(gb));
    if (jt_ == SYM(introSpriteTriforceSubid) && hook_is(gb, SYM(introSpriteTriforceSubid), introSpriteTriforceSubid_hook)) { introSpriteTriforceSubid_hook(gb); return; }
    else if (jt_ == SYM(introSpriteRunTriforceGlowSubid) && hook_is(gb, SYM(introSpriteRunTriforceGlowSubid), introSpriteRunTriforceGlowSubid_hook)) { introSpriteRunTriforceGlowSubid_hook(gb); return; }
    else if (jt_ == SYM(introSpriteRunSubid04) && hook_is(gb, SYM(introSpriteRunSubid04), introSpriteRunSubid04_hook)) { introSpriteRunSubid04_hook(gb); return; }
    else if (jt_ == SYM(introSpriteRunSubid07) && hook_is(gb, SYM(introSpriteRunSubid07), introSpriteRunSubid07_hook)) { introSpriteRunSubid07_hook(gb); return; }
    else if (jt_ == SYM(introSpriteRunSubid08) && hook_is(gb, SYM(introSpriteRunSubid08), introSpriteRunSubid08_hook)) { introSpriteRunSubid08_hook(gb); return; }
