...
void interactionCode11_hook(GB *gb) {
  ...
  do { uint16_t jt_ = (faroreMakeChest_jumpTable(gb));
    if (jt_ == SYM(interac11_subid00) && hook_is(gb, SYM(interac11_subid00), interac11_subid00_hook)) { interac11_subid00_hook(gb); return; }
    else if (jt_ == SYM(interac11_subid01) && hook_is(gb, SYM(interac11_subid01), interac11_subid01_hook)) { interac11_subid01_hook(gb); return; }
    else { HANDOFF(HL); }
  } while (0);
}
