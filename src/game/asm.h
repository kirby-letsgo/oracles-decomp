...
#define CALL(a, fn, target, ra) do { I((a), 5); push_timed(gb, (ra)); uint16_t sp_ = gb->sp; if (hook_in_verify || !hook_is(gb, target, fn)) asm_call(gb, (target), (ra)); else { hook_run(gb, fn, (target)); if (!(gb->pc == (ra) && gb->sp == (uint16_t)(sp_ + 2))) HANDOFF(gb->pc); } } while (0)
