...
state1:
  ...
  if (!(F & FC)) { CYCT(b_+14, b_+16); goto animate; }
  CYC(b_+14, b_+16); CYC(b_+16, b_+17); alu_xor(gb, A);
  CYC(b_+17, b_+20); W8(w1Link_direction) = A;
runScript:
  CALL_C(b_+20, interactionRunScript_hook, SYM(interactionRunScript), b_+23);
  if (F & FC) { CYCT(b_+23, b_+26); TAIL(vire_deleteAndReturnControl); }
  CYC(b_+23, b_+26);
animate:
...
state0:
  ...
  if (F & FZ) { CYCT(b_+26, b_+28); TAIL(vire_setScript); }
...
state2:
  ...
  if (!(F & FZ)) { RET_TAKEN(b_+84); return; } CYC(b_+84, b_+85);
  CYC(b_+85, b_+88); SET_HL(w1Link_yh); CYC(b_+88, b_+89); A = mem_rd(gb, HL); SET_HL(HL + 1); CYC(b_+89, b_+91); alu_cp(gb, 0x10);
  if (!(F & FC)) { CYCT(b_+91, b_+93); goto spawn; }
  CYC(b_+91, b_+93); CYC(b_+93, b_+94); L = alu_inc8(gb, L); CYC(b_+94, b_+95); A = mem_rd(gb, HL); CYC(b_+95, b_+97); alu_cp(gb, 0xa0);
  if (!(F & FC)) { CYCT(b_+97, b_+99); TAIL(vire_setRandomCounter1); }
  CYC(b_+97, b_+99);
spawn:
  CALL_C(b_+99, getFreePartSlot_hook, SYM(getFreePartSlot), b_+102);
  if (!(F & FZ)) { CYCT(b_+102, b_+104); TAIL(vire_setRandomCounter1); }
  CYC(b_+102, b_+104);
  CYC(b_+104, b_+106); mem_wr(gb, HL, 0x2c); CYC(b_+106, b_+107); L = alu_inc8(gb, L); CYC(b_+107, b_+108); mem_wr(gb, HL, alu_inc8(gb, mem_rd(gb, HL))); TAIL(vire_setRandomCounter1);
}
...
state0:
  ...
  CYC(b_+17, b_+20);
  CYC(b_+20, b_+23); SET_BC(0xad03);
  CALL_C(b_+23, objectCreateInteraction_hook, SYM(objectCreateInteraction), b_+26);
  if (!(F & FZ)) { RET_TAKEN(b_+26); return; } CYC(b_+26, b_+27);
  CYC(b_+27, b_+29); E = 0x56; CYC(b_+29, b_+31); A = 0x40; CYC(b_+31, b_+32); mem_wr(gb, DE, A);
  CYC(b_+32, b_+33); E = alu_inc8(gb, E); CYC(b_+33, b_+34); A = H; CYC(b_+34, b_+35); mem_wr(gb, DE, A);
  CYC(b_+35, b_+38); SET_HL((SYM(interactionCoded8__subid0Script) + 20));
  CALL_C(b_+38, vire_setScript_hook, SYM(vire_setScript), b_+41);
  CYC(b_+41, b_+43); L = 0x46; CYC(b_+43, b_+45); mem_wr(gb, HL, 0x08); RET(b_+45); return;

state1:
  CYC(b_+46, b_+49); SET_HL(w1Link_yh); CYC(b_+49, b_+50); A = mem_rd(gb, HL); SET_HL(HL + 1);
  CYC(b_+50, b_+52); alu_cp(gb, 0x40);
  if (!(F & FC)) { CYCT(b_+52, b_+54); goto game; }
  CYC(b_+52, b_+54);
  CYC(b_+54, b_+55); L = alu_inc8(gb, L); CYC(b_+55, b_+56); A = mem_rd(gb, HL); CYC(b_+56, b_+58); alu_cp(gb, 0x58);
  if (!(F & FC)) { CYCT(b_+58, b_+60); goto game; }
  CYC(b_+58, b_+60);
  CALL_C(b_+60, vire_disableObjectsIfLinkIsReady_hook, SYM(vire_disableObjectsIfLinkIsReady), b_+63);
  if (!(F & FC)) { CYCT(b_+63, b_+65); goto game; }
  CYC(b_+63, b_+65);
  CYC(b_+65, b_+67); A = 1; CYC(b_+67, b_+70); W8(wDisabledObjects) = A;
  CYC(b_+70, b_+73); W8(wTmpcfc0_fairyHideAndSeek_active) = A;
  CYC(b_+73, b_+75); A = 3; CYC(b_+75, b_+78); W8(w1Link_direction) = A;
  CYC(b_+78, b_+81); TAIL(interactionIncState);

game:
  CYC(b_+81, b_+82); H = D; CYC(b_+82, b_+84); L = 0x47; CYC(b_+84, b_+85); A = mem_rd(gb, HL); CYC(b_+85, b_+86); alu_or(gb, A);
  if (F & FZ) { CYCT(b_+86, b_+88); goto counter; }
  CYC(b_+86, b_+88);
  CYC(b_+88, b_+89); mem_wr(gb, HL, alu_dec8(gb, mem_rd(gb, HL)));
  if (!(F & FZ)) { CYCT(b_+89, b_+91); goto counter; }
  CYC(b_+89, b_+91);
  CYC(b_+91, b_+93); E = 0x48; CYC(b_+93, b_+94); alu_xor(gb, A); CYC(b_+94, b_+95); mem_wr(gb, DE, A);
...
counter:
  ...
  if (!(F & FZ)) { CYCT(b_+101, b_+103); goto animate2; }
  CYC(b_+101, b_+103);
  CALL_C(b_+103, getFreePartSlot_hook, SYM(getFreePartSlot), b_+106);
  if (!(F & FZ)) { CYCT(b_+106, b_+108); goto setCounter; }
  CYC(b_+106, b_+108);
  CYC(b_+108, b_+110); mem_wr(gb, HL, 0x2c);
  CALL_C(b_+110, objectCopyPosition_hook, SYM(objectCopyPosition), b_+113);
  CYC(b_+113, b_+115); E = 0x48; CYC(b_+115, b_+117); A = 1; CYC(b_+117, b_+118); mem_wr(gb, DE, A);
  CALL_C(b_+118, interactionSetAnimation_hook, SYM(interactionSetAnimation), b_+121);
  CYC(b_+121, b_+123); E = 0x47; CYC(b_+123, b_+125); A = 0x18; CYC(b_+125, b_+126); mem_wr(gb, DE, A);
setCounter:
...
state2:
  CALL_C(b_+132, interactionIncState_hook, SYM(interactionIncState), b_+135);
  CYC(b_+135, b_+137); L = 0x46; CYC(b_+137, b_+138); alu_xor(gb, A);
  CYC(b_+138, b_+139); mem_wr(gb, HL, A); SET_HL(HL + 1); CYC(b_+139, b_+140); mem_wr(gb, HL, A);
  CYC(b_+140, b_+142); E = 0x48; CYC(b_+142, b_+143); A = mem_rd(gb, DE); CYC(b_+143, b_+144); A = alu_dec8(gb, A);
  if (F & FZ) CALL_C_CC(b_+144, interactionSetAnimation_hook, SYM(interactionSetAnimation), b_+147); else CYC(b_+144, b_+147);
  CYC(b_+147, b_+149); A = 0x80; CYC(b_+149, b_+152); W8(wDisabledObjects) = A;
  CYC(b_+152, b_+154); A = 0xf0;
  CALL_C(b_+154, playSound_b00_hook, SYM(playSound_b00), b_+157);

state3:
  CALL_C(b_+157, interactionRunScript_hook, SYM(interactionRunScript), b_+160);
  if (!(F & FC)) { CYCT(b_+160, b_+162); goto animate2; }
  CYC(b_+160, b_+162);
  CYC(b_+162, b_+164); A = 5;
  CALL_C(b_+164, objectGetRelatedObject1Var_hook, SYM(objectGetRelatedObject1Var), b_+167);
  CYC(b_+167, b_+168); mem_wr(gb, HL, alu_inc8(gb, mem_rd(gb, HL)));
  CYC(b_+168, b_+171); TAIL(interactionDelete);
}

void vire_disableObjectsIfLinkIsReady_hook(GB *gb) {
  BASE(vire_disableObjectsIfLinkIsReady);
  uint16_t sp0_ = gb->sp;
  CYC(b_+0, b_+3); A = mem_rd(gb, wLinkInAir); CYC(b_+3, b_+4); alu_or(gb, A); if (!(F & FZ)) { RET_TAKEN(b_+4); return; } CYC(b_+4, b_+5);
  CALL_C(b_+5, checkLinkVulnerable_hook, SYM(checkLinkVulnerable), b_+8); if (!(F & FC)) { RET_TAKEN(b_+8); return; } CYC(b_+8, b_+9);
