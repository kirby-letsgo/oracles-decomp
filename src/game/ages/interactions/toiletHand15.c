...
check_position:
  ...
  CYC(b_+30, b_+33); TAIL(writeFlagsTocddb); // jp
}
...
void toiletHand_retreatIntoToiletIfNotAlready_hook(GB *gb) {
  ...
  CYC(b_+5, b_+6);
  TAIL(toiletHand_retreatIntoToilet); // fallthrough
}
...
void toiletHand_retreatIntoToilet_hook(GB *gb) {
  ...
  CYC(b_+2, b_+4); TAIL(toiletHand_setAnimation); // jr
}
...
void toiletHand_disappear_hook(GB *gb) {
  ...
  TAIL(toiletHand_setAnimation); // fallthrough
}
