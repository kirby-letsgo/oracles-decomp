...
static void toilet_hand_load_script(GB *gb, uint16_t sp0_) {
  ...
  CYC(b_+106, b_+108); E = INTERACTION_BASE + OBJ_SUBID; CYC(b_+108, b_+109); A = mem_rd(gb, DE); CYC(b_+109, b_+112); SET_HL(b_+184); CYC(b_+112, b_+113); toilet_hand_add_double_index(gb, b_+113); CYC(b_+113, b_+114); A = mem_rd(gb, HL); SET_HL(HL + 1); CYC(b_+114, b_+115); H = mem_rd(gb, HL); CYC(b_+115, b_+116); L = A; CYC(b_+116, b_+119); interactionSetScript_hook(gb);
}
...
static void toilet_hand_respond_to_object_in_hole(GB *gb, uint16_t sp0_) {
  ...
  CYC(b_+143, b_+146); A = mem_rd(gb, wTmpcfc0_fallDownHoleEvent + 0x19); CYC(b_+146, b_+147); E = A; CALL_C(b_+147, lookupKey_hook, SYM(lookupKey), b_+150); if (!(F & FC)) { CYCT(b_+150, b_+151); ret_effect(gb); return; } CYC(b_+150, b_+151); CYC(b_+151, b_+153); E = INTERACTION_BASE + OBJ_VAR38; CYC(b_+153, b_+154); mem_wr(gb, DE, A); CYC(b_+154, b_+155); ret_effect(gb);
}
...
state1:
  ...
  CALL_C(b_+29, interactionRunScript_hook, SYM(interactionRunScript), b_+32); CYC(b_+32, b_+33); H = D; CYC(b_+33, b_+35); L = INTERACTION_BASE + OBJ_VISIBLE; CYC(b_+35, b_+37); alu_bit(gb, 7, mem_rd(gb, HL)); if (F & FZ) { CYCT(b_+37, b_+38); ret_effect(gb); return; } CYC(b_+37, b_+38); CYC(b_+38, b_+41); interactionAnimateAsNpc_hook(gb); return;
dropped:
...
state2:
  CYC(b_+50, b_+53); A = W8(wTextIsActive); CYC(b_+53, b_+54); alu_or(gb, A); if (!(F & FZ)) { CYCT(b_+54, b_+55); ret_effect(gb); return; } CYC(b_+54, b_+55); CALL_C(b_+55, interactionRunScript_hook, SYM(interactionRunScript), b_+58); if (F & FC) goto script_ended; CYC(b_+58, b_+60); CYC(b_+60, b_+61); H = D; CYC(b_+61, b_+63); L = INTERACTION_BASE + OBJ_VISIBLE; CYC(b_+63, b_+65); alu_bit(gb, 7, mem_rd(gb, HL)); if (F & FZ) { CYCT(b_+65, b_+66); ret_effect(gb); return; } CYC(b_+65, b_+66); CALL_C(b_+66, interactionAnimateAsNpc_hook, SYM(interactionAnimateAsNpc), b_+69); CYC(b_+69, b_+72); interactionAnimate_hook(gb); return;
script_ended:
  CYCT(b_+58, b_+60); CYC(b_+72, b_+75); push_effect(gb, b_+75); toilet_hand_load_script(gb, sp0_); CYC(b_+75, b_+78); SET_HL((SYM(shopkeeperState3) + 3)); CYC(b_+78, b_+80); E = 0x08; CALL_C(b_+80, interBankCall_hook, 0x008a, b_+83); CYC(b_+83, b_+85); E = INTERACTION_BASE + OBJ_STATE; CYC(b_+85, b_+87); A = 1; CYC(b_+87, b_+88); mem_wr(gb, DE, A); CYC(b_+88, b_+89); ret_effect(gb);
}
