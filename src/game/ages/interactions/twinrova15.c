...
#undef CYC
#undef CYCT
#define CYC(from, to) burn_rom(gb, SYMBANK(objectWritePositionTocfd5), (from), (to), false)
#define CYCT(from, to) burn_rom(gb, SYMBANK(objectWritePositionTocfd5), (from), (to), true)
...
void objectWritePositionTocfd5_hook(GB *gb) {
  ...
  CYC(b_+0, SYM(twinrova_writePositionWithXOffsetTocfd5)); alu_xor(gb, A);
...
void twinrova_writePositionWithXOffsetTocfd5_hook(GB *gb) {
  ...
  CYC(b_+5, b_+7); A = hram_rd(gb, 0x8b);
  ...
  CYC(b_+14, SYM(twinrova_subid00Script_body)); ret_effect(gb);
}
