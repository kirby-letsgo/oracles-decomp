...
#undef CYC
#undef CYCT
#define CYC(from, to) burn_rom(gb, SYMBANK(enemyCode1f), (from), (to), false)
#define CYCT(from, to) burn_rom(gb, SYMBANK(enemyCode1f), (from), (to), true)
...
stateA:
  ...
  CYC(b_+104, SYM(enemyCode26)); enemyDelete_hook(gb); return; // jp
}
