...
#undef CYC
#undef CYCT
#define CYC(from, to) burn_rom(gb, SYMBANK(enemyCode63), (from), (to), false)
#define CYCT(from, to) burn_rom(gb, SYMBANK(enemyCode63), (from), (to), true)
...
skipInitSpeed:
  CYC(b_+14, SYM(targetCartCrystal_state1)); objectSetVisible80_hook(gb); return; // jp
}
...
animate:
  CYC(b_+23, SYM(targetCartCrystal_state2)); enemyAnimate_hook(gb); return; // jp
}
...
delete:
  CYC(b_+42, SYM(targetCartCrystal_loadBehaviour)); enemyDelete_hook(gb); return; // jp
}
...
applySpeed:
  CYC(b_+13, SYM(movingSidescrollPlatformScriptTable)); objectApplySpeed_hook(gb); return; // jp
}
